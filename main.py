...
def open_chat(driver: webdriver.Chrome, phone: str, message: str, timeout: int = 35) -> Tuple[bool, str]:
    ...
    try:
        ...
        return False, "Timeout opening chat"
...
def send_attachment(driver: webdriver.Chrome, file_path: str, timeout: int = 35) -> Tuple[bool, str]:
    """Attach and send a local file (image/video/document)."""
    if not file_path:
    ...
    try:
        ...
        file_input = WebDriverWait(driver, timeout).until(
            EC.presence_of_element_located((By.XPATH, "(//input[@type='file'])[last()]"))
        )
        ...
        return False, "Attachment failed: timeout waiting for UI"
...
def process_rows(
    ...
    try:
        ...
        for row in range(2, sheet.max_row + 1):
            ...
            ok, reason = open_chat(driver, phone, message)
            if not ok:
            ...
            media_ok, media_reason = send_attachment(driver, attachment)

            if not txt_ok and not attachment:
                final_status = f"Failed: {txt_reason}"
            elif attachment:
                if txt_ok and media_ok:
                    final_status = "Sent text + attachment"
                elif txt_ok and not media_ok:
                    final_status = f"Sent text, attachment failed ({media_reason})"
                elif not txt_ok and media_ok:
                    final_status = f"Attachment sent, text failed ({txt_reason})"
                else:
                    final_status = f"Failed: text ({txt_reason}) + attachment ({media_reason})"
            else:
                final_status = "Sent text"
